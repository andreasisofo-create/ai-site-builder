...
import { useLanguage } from "@/lib/i18n"; // Assuming this exists from previous file
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    // Observer for form visibility to hide sticky button
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white overflow-x-hidden font-sans">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <Link href="/" className="flex items-center">
            {/* Logo Placeholder - ideal to fetch requested logo */}
            <span className="text-2xl font-bold tracking-tight text-white">
              E-quipe<span className="text-[#FF6B35]">.it</span>
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            {/* Language Switcher if needed, preserving previous logic */}
            <button
              onClick={scrollToForm}
              className="px-6 py-2.5 bg-[#FF6B35] hover:bg-[#FF8B5E] text-white rounded-lg font-bold text-sm transition-all shadow-lg hover:-translate-y-0.5"
            >
              Contattaci
            </button>
          </div>
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="px-6 py-8 space-y-6 flex flex-col items-center">
                ...
                <button
                  onClick={scrollToForm}
                  className="w-full py-4 bg-[#FF6B35] text-white rounded-xl font-bold text-lg"
                >
                  Contattaci
                </button>
              </div>
      ...
      {/* Sticky Mobile Bottom Bar (CTA always visible on mobile) */}
      {!isFormVisible && (
        <div className="lg:hidden fixed bottom-6 left-6 right-6 z-40 pointer-events-none">
          <button
            onClick={scrollToForm}
            className="w-full py-4 bg-[#FF6B35] text-white rounded-xl font-bold text-lg shadow-2xl pointer-events-auto shadow-orange-900/40 border border-white/10 backdrop-blur-md"
          >
            Contattaci — è gratis
          </button>
        </div>
